            ...
            if is_manager:
                ...
                data.update({
                    ...
                    **self._get_leave_data(employee, start_date, end_date),
                    ...
                    'user_id': employee.user_id.id if employee.user_id else False,  # <-- ADD THIS
                })
            return data
        except Exception:
            return self._get_empty_data()
...
class EmployeeDashboard(models.Model):
    ...
    def get_employee_stats(self):
        ...
        present_employees = attendance_model.search_count([
            ('check_in', '>=', start_of_day.strftime('%Y-%m-%d %H:%M:%S')),
            ('check_in', '<=', end_of_day.strftime('%Y-%m-%d %H:%M:%S')),
        ])
        currently_active = attendance_model.search_count([
            ('check_in', '!=', False),
            ('check_out', '=', False),
        ])
