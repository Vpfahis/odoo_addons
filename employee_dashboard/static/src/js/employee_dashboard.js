...
class Dashboard extends Component {
    ...
    setup() {
        ...
        onMounted(async() => {
            ...
            this._fetch_data();
            await this.loadStats();  // ← Add this line
    ...
    async loadStats() {
        const result = await this.orm.call("hr.employee", "get_employee_stats", []);
        this.state.employeeStats = result;
    }
    ...
    onClickActiveEmployees() {
        this.action.doAction({
            type: 'ir.actions.act_window',
            name: 'Currently Active Employees',
            res_model: 'hr.employee',
            domain: [['attendance_ids.check_out', '=', false]],
            views: [[false, 'kanban'], [false, 'form']],
            target: 'current',
        });
    }
